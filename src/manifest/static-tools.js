...
const staticTools = [
  ...
  },
  {
    ...
    description: "Search for contacts and return their full records. Does NOT return a count — use get_contact_count for that. Supports field-value conditions (e.g., last name equals 'Smith'), tag filters, and free-text search across all fields. Results are paginated with a maximum of 50 per request.",
    inputSchema: {
      type: 'object',
      properties: {
        ...conditionProperty,
  ...
  },
  {
    ...
    description: "Count how many contacts match optional criteria. Returns only the count, not the contact records. This is the correct tool when you need to know how many contacts exist or match a filter — do not use search_contacts for counting.",
    inputSchema: {
      type: 'object',
      properties: {
        ...conditionProperty,
  ...
  {
    name: 'create_invoice',
    description: "Create and optionally send an invoice (unpaid bill) to a contact. Unlike process_transaction, this does NOT charge a card — it creates an unpaid transaction and can email the contact a payment link. The contact pays the invoice through the payment page. Use pay_invoice to mark it paid via the API with a card on file.",
    inputSchema: {
      type: 'object',
      properties: {
        contact_id: { type: 'integer', description: 'Contact ID to invoice.' },
        gateway_id: { type: 'integer', description: 'Payment gateway ID for the payment form.' },
        offer: {
          type: 'object',
          description: 'Offer with products. Same structure as process_transaction but simpler — typically just { products: [{ id, quantity, owner }] }.',
        },
        send_invoice: { type: 'boolean', description: 'Whether to email the invoice to the contact. Defaults to false.' },
        due_on: { type: 'integer', description: 'Number of days after the transaction date that the invoice is due.' },
        trans_date: { type: 'integer', description: 'Invoice date in MILLISECONDS since Unix Epoch.' },
        customer_note: { type: 'string', description: 'Customer-facing note displayed on the invoice.' },
        internal_note: { type: 'string', description: 'Internal note attached to the invoice (not visible to customer).' },
        to_field: { type: 'string', description: "Email field on the contact to send the invoice to. Default: 'email'." },
        from_fields: {
          type: 'object',
          description: "Sender configuration. Example: { from: 'owner' }.",
        },
      },
      required: ['contact_id', 'gateway_id', 'offer'],
    },
  ...
  {
    name: 'pay_invoice',
    description: "Pay an existing unpaid invoice using a card on file. This charges the contact's saved card and marks the invoice as paid. If cc_id is omitted, the contact's default card is used.",
    inputSchema: {
      type: 'object',
      properties: {
        invoice_id: { type: 'integer', description: 'The invoice (transaction) ID to pay.' },
        cc_id: { type: 'integer', description: 'Credit card on file ID. If omitted, uses the default card.' },
      },
      required: ['invoice_id'],
    },
    module: 'transactions',
  ...
  {
    name: 'update_order',
    description: "Update an existing order's payment schedule by modifying its offer data. WARNING: You must include the full offer data — if offer data is omitted, the order will be deleted. Workflow: (1) get_order to find the order's offer_id and order id, (2) retrieve the current offer by fetching GET /object with objectID=65 and id=offer_id — the offer's 'data' field is a JSON string containing the full billing config (products, prices, subscriptions), (3) parse the JSON, modify what you need, (4) pass it back to update_order with offer_id and order_id included. Common modifications: change cc_id to switch the card on file, change the price in the products array to adjust billing amount, change unit/payment_count to modify billing cycle. Every order has an associated offer (objectID=65) — this is created automatically when the transaction is processed.",
    inputSchema: {
      type: 'object',
      properties: {
        offer: {
  ...
  },
  {
    ...
    description: "Count how many records exist in any object collection matching optional criteria. Returns only the count, not the records. For contacts specifically, you can also use get_contact_count (objectID 0). Use this for pagination planning or dashboard-style counts on any object type.",
    inputSchema: {
      type: 'object',
      properties: {
        objectID: { type: 'integer', description: 'Object type ID.' },
